...
required_packages <- c("dplyr", "ggplot2", "paletteer", "stringr", "readr","readxl", "glue",
                       "forcats", "tidyr",
                       "highcharter", "scales",  "plotly", "lubridate", "here","ISOweek", "countrycode")
...
library(pacman)
required_packages <- c("dplyr", "tidyr", "ggplot2", "paletteer", "stringr", "readr","readxl", "glue",
                       "highcharter", "forcats", "scales",  "plotly", "lubridate", "here", "ISOweek", "countrycode")
...
baselined_data <- most_affected_data %>% filter(Confirmed>=100000) %>% group_by(Country_Region) %>% 
  mutate(Days_since_100k=difftime(Date,first(Date), units = "days")) %>% ungroup()
# create the plot
plot <- ggplot(baselined_data, aes(x=Days_since_100k, y=Confirmed, colour=Country)) +
  geom_line(size=0.6) + scale_y_log10(labels=comma) +
    scale_color_paletteer_d("ggsci::springfield_simpsons") +
...
worst_by_pop_data <- covid_weekly_data %>% group_by(Country) %>% arrange(desc(Date)) %>% slice(1) %>% 
  ungroup() %>% filter(population>100000) %>% arrange(desc(Cases_per_mill)) %>% slice(1:11) 
...
Check at the following [BBC site](https://www.bbc.com/news/world-52103747) and check roughly when lockdowns and travel restrictions were applied.
...
cont_plot <- ggplot(cont_weekly_data, aes(x=Date, y=Total_cases, colour=continent)) +
  geom_line(size=0.6) + 
  geom_vline(xintercept = as.Date("2020-04-20"), col="grey20", linetype="dotdashed") + 
